                        ...
                        {ready && authenticated ? (
                            <DropdownMenu onOpenChange={setIsMenuOpen}>
                                <DropdownMenuTrigger asChild>
                                    <Avatar className="size-8 md:size-12 cursor-pointer ring-2 ring-[#9c60a0] transition-all hover:ring-[#f39c12]">
                                        <AvatarImage src={avatarImage} alt="" />
                                        <AvatarFallback>
                                            {user?.wallet?.address ? user.wallet.address.slice(0, 4) : 'gm'}
                                        </AvatarFallback>
                                    </Avatar>
                                </DropdownMenuTrigger>
                                <DropdownMenuContent align="end" className="w-56 z-[60]">
                                    ...
                                    <DropdownMenuItem>
                                        ...
                                        <span>{user?.wallet?.address ? `${user.wallet.address.slice(0, 4)}...${user.wallet.address.slice(-5)}` : ''}</span>
                                    </DropdownMenuItem>
                                    {hasEmbeddedWallet && (
                            ...
                            <Button disabled={disableLogin} onClick={login} className="ml-auto text-white bg-transparent rounded-xl transition-all duration-300 ease-in-out cursor-pointer gradient-button">
